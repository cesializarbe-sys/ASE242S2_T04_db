...
Tú dijiste:
# Base de Datos – SUMAK MIKUY

## 1. Creación de la base de datos
sql
DROP DATABASE IF EXISTS sumak_mikuy;
CREATE DATABASE sumak_mikuy CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE sumak_mikuy; entregame todo asi no lo separes porque al copiar y pegar se ve desordenado y no se entiende


ChatGPT Plus
¡Perfecto! Aquí tienes todo el contenido del script de implementación y explicación, listo para copiar y pegar como un solo bloque en tu info-development.md, sin separaciones que rompan el formato:
...
USE sumak_mikuy;
Explicación:
Se elimina la base de datos anterior si existe, se crea la base de datos sumak_mikuy con codificación UTF-8 (utf8mb4) para soportar caracteres especiales, y se selecciona la base de datos recién creada para trabajar sobre ella.
...
('María', 'González', 'maria.gonzalez@hotmail.com', 'hashed_pass', 'Jr. Miraflores 456', '912345678', 'google', 'activo', 'admin');
Explicación:
Almacena los usuarios del sistema: clientes y personal. rol define permisos y provider indica cómo se registró el usuario. Índices para búsquedas rápidas.
...
(2,2,'interior','Mesa íntima para parejas','disponible');
Explicación:
Registra mesas, su capacidad y ubicación. status controla la disponibilidad. Índices para filtrar por estado o número de mesa.
...
('Platos Principales','Platos típicos peruanos',2,'activo');
Explicación:
Categorías de platos del restaurante. orden indica posición en el menú y status permite activar o desactivar.
...
(2,'Lomo saltado','Carne de res, cebolla y papas fritas',35.00,25,'Carne de res, cebolla, tomate, papas, arroz',FALSE,FALSE,'disponible');
Explicación:
Contiene los platos del menú y su categoría. es_vegetariano y es_picante permiten clasificar platos. Índices para búsquedas por categoría y estado.
...
(1,3,6,'2025-10-10','19:00:00','Cumpleaños', 'confirmada');
Explicación:
Registra reservas de clientes. usuario_id y mesa_id enlazan con usuarios y mesas. status indica estado de la reserva.
...
(1,3,1,180.00,'tarjeta','Sin cebolla','entregado');
Explicación:
Registra pedidos realizados. reserva_id vincula con reservas previas. metodo_pago y status permiten controlar operaciones y pagos.
...
(1,1,2,25.00,'Término medio','entregado');
Explicación:
Contiene los detalles de cada pedido: platos, cantidad, precio unitario y subtotal calculado automáticamente.

4. Resumen de Relaciones
usuarios → reservas y pedidos (un usuario puede tener muchas reservas o pedidos).

mesas → reservas y pedidos (una mesa puede tener muchas reservas/pedidos).

categorias_menu → platos (una categoría puede tener muchos platos).

platos → detalle_pedidos (un plato puede estar en muchos pedidos).

pedidos → detalle_pedidos (un pedido puede tener muchos detalles).